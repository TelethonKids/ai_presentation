...
- How specific must prompts before a model can be output and coefficients interpreted?
...
- Simulated cohort of lung cancer patients.
...
hpd %>%
  select(remission, Age, IL6, CRP, LengthofStay) %>%
  tbl_summary(by = remission) %>%
  italicize_labels() %>%
  modify_spanning_header(all_stat_cols() ~ "**Remission (N = {N})**")
...
```{r echo=FALSE, out.width = '70%', fig.align='center'}
knitr::include_graphics("images/log_reg_1.png")
```
...
```{r echo=FALSE, out.width = '70%', fig.align='center'}
knitr::include_graphics("images/log_res_2.png")
```
...
```{r echo=FALSE, out.width = '70%', fig.align='center'}
knitr::include_graphics("images/log_res_3.png")
```
...
- Answers appear **informed and accurate**.
