...
- unless the AI resides solely on the server that your data were approved to reside on (within your study documentation)
...
```{r echo=FALSE, out.height = '15%', fig.align='center'}
...
+ How effective could AI tools be to inform and guide data analysis?
...
+ **Types of decisions:**
  ...
  + How to account for model complexities (e.g., complex correlation nesting)?
  + How can output be interpreted? How could this be visualised?
  
---

# Example

+ Consider simulated data from the [UCLA Office of Advanced Research Computing](https://stats.idre.ucla.edu/stat/data/hdp.csv).
+ Cohort of lung cancer patients.
+ We want to explore how patient- and physician-level factors relate to lung cancer remission following treatment.

-- 

+ How specific/informed must prompts be to make appropriate decisions?
...
## Data QC & Variable Transformation
...
+ If I am new to things, how can I be sure this is correct? By knowig how to verify this, I should know what to do anyway
...
ggplot() +
  ...
  theme(axis.title.x = element_blank(),   # Remove x-axis label
        ...
        text=element_text(size=21))   # Remove x-axis ticks
```
...
my_colors <- c("Expected AI capability" = "red", "Zigzag Line" = "blue", "Points" = "black")
...
- Two projects devised, one inside the frontier, the other outside.
...
- Participants further randomised to either "ChatGPT Only" or "ChatGPT + Overview"
...
data.frame(
  ...
  kable() %>% kable_styling() %>%
  kableExtra::pack_rows("ChatGPT Only", 1, 2) %>%
...
ggplot() +
  ...
  # Add points
  annotation_custom(gradient_bg, -Inf, Inf, -Inf, Inf) +
  geom_point(data = points_data, aes(x = x, y = y), size = 7) +
  # Add labels and customize axes
  labs(y = "Perceived Task Difficulty",
       color = "Legend") +
...
- How can we know where the frontier is?

--

- How do we assess the quality of the output? Does this depend on the type of output?
